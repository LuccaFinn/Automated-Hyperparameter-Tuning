...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of steps for numeric ranges"
    )
    args = parser.parse_args()
    
    algorithm = args.algorithm
    print(f"==================================================")
    print(f"Grid Search für {algorithm.upper()} gestartet")
    print(f"==================================================")
    ...
    print("Grid Search gestartet um: " + starttime.strftime('%H:%M:%S'))
    
    best_params, best_score, best_model = tuner.tune(search_space)
    
    finishtime = datetime.now()
    print("Grid Search beendet um: " + finishtime.strftime('%H:%M:%S'))
